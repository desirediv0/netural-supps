...
export const getMostViewedProducts = asyncHandler(async (req, res) => {
  ...
  const formattedProductViews = await Promise.all(
    productViews.map(async (view) => {
      ...
      // Get unique flavor and weight counts
      const uniqueFlavors = new Set(
        product.variants.filter((v) => v.flavor).map((v) => v.flavorId)
      );
      const uniqueWeights = new Set(
        product.variants.filter((v) => v.weight).map((v) => v.weightId)
      );
      ...
      return {
        ...
        product: {
          ...
          variantInfo: {
            flavors: uniqueFlavors.size,
            weights: uniqueWeights.size,
          },
        },
      };
    })
      ...
      return {
        ...
        product: {
          ...
          variant: {
            ...
            flavor: item.productVariant.flavor?.name || null,
            flavorImage: item.productVariant.flavor?.image
              ? getFileUrl(item.productVariant.flavor.image)
              : null,
            weight: item.productVariant.weight
              ? `${item.productVariant.weight.value}${item.productVariant.weight.unit}`
              : null,
